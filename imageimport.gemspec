# coding: utf-8
lib = File.expand_path('../lib', __FILE__)
$LOAD_PATH.unshift(lib) unless $LOAD_PATH.include?(lib)
require 'imageimport/version'

Gem::Specification.new do |spec|
  spec.name          = "imageimport"
  spec.version       = ImageImport::VERSION
  spec.authors       = ["Mark Keisler"]
  spec.email         = ["grimm26@gmail.com"]
  spec.summary       = %q{Watch a directory for photos to import.}
  spec.description   = %q{Use inotify to watch a directory to import new images into a directory structure based on date/time the photo was taken.}
  spec.homepage      = ""
  spec.license       = "MIT"

  spec.files         = `git ls-files -z`.split("\x0")
  spec.executables   = spec.files.grep(%r{^bin/}) { |f| File.basename(f) }
  spec.test_files    = spec.files.grep(%r{^(test|spec|features)/})
  spec.require_paths = ["lib"]

  spec.add_development_dependency "bundler", "~> 1.5"
  spec.add_development_dependency "rake"
  spec.add_development_dependency "rspec"
  spec.add_dependency "rb-inotify"
  spec.add_dependency "filemagic"
  spec.add_dependency "exifr"

end
